...
const slides = [
  ...
  'https://images.unsplash.com/photo-1605745341112-85968b19335b?w=1920&q=80',
];
...
export default function Hero() {
  ...
  return (
    <section id="home" className="relative h-screen min-h-[700px] overflow-hidden flex items-center">
      ...
      {/* Slider Dots */}
      <div className="absolute bottom-28 right-[5%] z-[3] flex gap-3">
        {slides.map((_, i) => (
          <button
            key={i}
            onClick={() => setCurrentSlide(i)}
            className={`w-3.5 h-3.5 rounded-full border-2 transition-all relative ${
              currentSlide === i ? 'border-gold' : 'border-white/50'
            }`}
            aria-label={`Slide ${i + 1}`}
          >
            {currentSlide === i && (
              <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-gold rounded-full" />
            )}
          </button>
        ))}
      </div>
